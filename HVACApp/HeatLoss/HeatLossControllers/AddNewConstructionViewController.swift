...
class AddNewConstructionViewController: UIViewController {
    ...
    private lazy var saveButton: UIButton = {
        ...
        buttom.addTarget(self, action: #selector(saveAction(_:)), for: .touchUpInside)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let orientation = currentConstruction?.orientation {
            if let index = orientationAndNameArray[0].firstIndex(of: orientation) {
        ...
        if let constructionName = currentConstruction?.name {
            if let index = orientationAndNameArray[1].firstIndex(of: constructionName) {
    ...
    @objc private func saveAction(_ sender: UIButton) {
        let row1 = orientationAndNamePickerView.selectedRow(inComponent: 0)
        let row2 = orientationAndNamePickerView.selectedRow(inComponent: 1)
        
        let orientation = orientationAndNameArray[0][row1]
