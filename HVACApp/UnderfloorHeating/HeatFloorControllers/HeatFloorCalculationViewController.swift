...
class HeatFloorCalculationViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    @objc private func calculateAction(sender: UIButton) {
        let pipeManufacturer = pipeManufacturerArray[pipeManufacturerPickerView.selectedRow(inComponent: 0)]
        let floorConstruction = floorConstructionArray[floorConstructionPickerView.selectedRow(inComponent: 0)]
        let waterTemperature = Double(waterTemperatureArray[temperaturePickerView.selectedRow(inComponent: 0)])!
        
        let distanceBetweenPipes = Double(distanceBetweenPipesArray[distanceBetweenPipesPickerView.selectedRow(inComponent: 0)])!
        let indoorTemperature = Double(indoorTemperatureArray[indoorTemperaturePickerView.selectedRow(inComponent: 0)])!
    ...
    func saveDefaults() {
        ...
        defaults.set(distanceBetweenPipesPickerView.selectedRow(inComponent: 0), forKey: "distanceBetweenPipesIndex")
        defaults.set(indoorTemperaturePickerView.selectedRow(inComponent: 0), forKey: "indoorTemperatureIndex")
    }
